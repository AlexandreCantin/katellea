...
const CONF_FILE_PATH = './conf/settings/settings.json';
...
class EnvironmentValuesLoader {
  ...
  static init() {
    ...
    if (!fs.existsSync(CONF_FILE_PATH)) {
      throw new Error(`The file ${CONF_FILE_PATH} does not exists. Please create one :-)`);
    }
    ...
    if (pjson.environmentOverrideFile && fs.existsSync(pjson.environmentOverrideFile)) {
      overridesEnvironmentValues = JSON.parse(fs.readFileSync(pjson.environmentOverrideFile));
    }
